...
const AdminChat = () => {
    ...
    const [selectedChat] = useState("admin");
    const [userId] = useState(() => localStorage.getItem("aweUserId"));
    const socketRef = useRef(null);

    useEffect(() => {
        const socket = new WebSocket(`wss://ivalukyan-backend-damnitbot-12c1.twc1.net/user/chat/${userId}`);
    ...
    return (
        <>
            <nav className="navbar">
                <a className="navbar-brand" href="/admin/chats">
                    <svg
            ...
            <div className="chat-container">
                <div className="chat-body">
                    {messages.map((msg, index) => (
                        <div key={index} className={`${msg.role === "admin" ? "sent" : "receive"}`}>
                            <p>{msg.content}</p>
                        </div>
                    ))}
                </div>
